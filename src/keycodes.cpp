/*
 * This lookup table was generated by first accessing the documentation for virtual keys:
 *
 * https://docs.microsoft.com/en-us/windows/win32/inputdev/virtual-key-codes
 *
 * Then executing this javascript snippet:

[...document.querySelector("#main > div:nth-child(5) > table > tbody").children].map(elem =>
    ({
        top: elem.children[0].children[elem.children[0].children.length-1].children[0].innerText,
        bottom: elem.children[0].children[elem.children[0].children.length-1].children[1].innerText,
        description: elem.children[1].innerText.trim()
    })
).filter(obj =>
    !(["Undefined", "Unassigned", "Reserved", "OEM specific"].includes(obj.description))
).map(obj =>
    ({
        value: obj.top === "" ? "VK_" + obj.description.split(" ")[0] : obj.top,
        code: obj.bottom,
        comment: obj.description
    })
).map(
    obj => `    ${String.fromCharCode(47)}* ${obj.code} *${String.fromCharCode(47)} "${obj.value}", // ${obj.comment}`
).join("\n");

*/

char KEYCODE_LOOKUP_TABLE[32][] = {
